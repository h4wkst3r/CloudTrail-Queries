...
WHERE 
    e1.eventName IN ('ListNotebookInstances', 'UpdateNotebookInstance', 'CreateNotebookInstanceLifecycleConfig', 'StopNotebookInstance', 'StartNotebookInstance')
    AND e2.eventName IN ('ListNotebookInstances', 'UpdateNotebookInstance', 'CreateNotebookInstanceLifecycleConfig', 'StopNotebookInstance', 'StartNotebookInstance')
    AND e1.userIdentity.arn <> ''
GROUP BY 
    e1.userIdentity.arn, 
    e1.sourceIpAddress
HAVING 
    COUNT(DISTINCT e1.eventName) = 5
ORDER BY 
